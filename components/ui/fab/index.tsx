...
const UIFab = createFab({
  Root:
    Platform.OS === 'web'
      ? withStyleContext(TouchableOpacity, SCOPE)
      : withStyleContextAndStates(TouchableOpacity, SCOPE),
  Label: Text,
  Icon: PrimitiveIcon,
});
...
const fabStyle = tva({
  base: 'group/fab disabled:pointer-events-all absolute z-20 flex-row items-center justify-center rounded-full border border-beige bg-brown p-4 shadow-hard-2 disabled:cursor-not-allowed disabled:opacity-40 data-[focus=true]:web:outline-none data-[focus-visible=true]:web:ring-2 data-[focus-visible=true]:web:ring-indicator-info',
  variants: {
    size: {
      sm: 'px-2.5 py-2.5',
      md: 'px-3 py-3',
